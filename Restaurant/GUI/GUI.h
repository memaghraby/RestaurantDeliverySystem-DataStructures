...
private:
	...
	static const int	
		WindWidth=1200, WindHeight=800,		//Window width and height
		StatusBarHeight = 410,	//Status Bar Height
...
public:
	...
	void PrintMessageCoordinates(const int iX, const int iY, const string Text);

	void UpdateInterface();
	void AddOrderForDrawing(Order* pOrd);	//Adds a new order to the drawing list
	void RemoveOrderForDrawing(int id);
	void DeleteServedOrder(int id);
	int GetOrderCount();
