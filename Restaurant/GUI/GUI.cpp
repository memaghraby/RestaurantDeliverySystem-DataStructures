...
void GUI::PrintMessage(string msg) const	//Prints a message on status bar
{
	...
	pWind->DrawString(10, 410, msg); // You may need to change these coordinates later 
	
	                                                                    
}


void GUI::PrintMessageCoordinates(const int iX, const int iY, const string Text)
{
...
void GUI::AddOrderForDrawing(Order* ptr)
{
	...
	{
		switch (ptr->Getordertype())
		{
		case TYPE_VIP:
			for (int i = 0; i < OrderCount; i++){
				if (OrdListForDrawing[i]->Getordertype() == TYPE_FROZ || OrdListForDrawing[i]->Getordertype() == TYPE_NRM){
					for (int j = OrderCount-1; j >= i; j--){
						OrdListForDrawing[j + 1] = OrdListForDrawing[j];
					}
		...
		case TYPE_FROZ:
			for (int i = 0; i < OrderCount; i++){
				if (OrdListForDrawing[i]->Getordertype() == TYPE_NRM){
					for (int j = OrderCount - 1; j >= i; j--){
						OrdListForDrawing[j + 1] = OrdListForDrawing[j];
					}
					OrdListForDrawing[i] = ptr;
...
void GUI::RemoveOrderForDrawing(int id)
{
	for (int i = 0; i < OrderCount; i++)
	{
...
void GUI::DeleteServedOrder(int id)
{
	for (int i = 0; i < OrderCount; i++)
	{
		if (OrdListForDrawing[i]->GetID() == id)
